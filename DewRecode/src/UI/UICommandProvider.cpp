...
namespace UI
{
	...
	bool UICommandProvider::CommandShowH3UI(const std::vector<std::string>& Arguments, std::string& returnInfo)
	{
		if (Arguments.size() <= 0)
		{
	...
	bool UICommandProvider::CommandSettingsMenu(const std::vector<std::string>& Arguments, std::string& returnInfo)
	{
	...
	{
		auto& dorito = ElDorito::Instance();
		dorito.CommandManager.Execute(boxTag + " \"" + result + "\"");
	...
	{
		ElDorito::Instance().CommandManager.Execute(boxTag + " \"" + result + "\"");
	}
}