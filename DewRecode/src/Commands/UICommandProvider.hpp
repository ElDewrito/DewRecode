...
namespace UI
{
	...
	public:
		...
		bool CommandShowH3UI(const std::vector<std::string>& Arguments, ICommandContext& context);
		void ShowH3UI(uint32_t stringId, int32_t arg1, int32_t flags, uint32_t parentStringId);

		bool CommandSettingsMenu(const std::vector<std::string>& Arguments, ICommandContext& context);
