...
namespace Input
{
	...
	bool InputCommandProvider::VariableRawInputUpdate(const std::vector<std::string>& Arguments, ICommandContext& context)
	{
		...
		context.WriteOutput("Raw input " + status);
		return true;
	}
	...
	bool InputCommandProvider::CommandBind(const std::vector<std::string>& Arguments, ICommandContext& context)
	{
		if (Arguments.size() < 1)
		{
			context.WriteOutput("Usage: Bind <key> [[+]command] [arguments]\n");
			context.WriteOutput("If the command starts with a +, then it will be ");
			context.WriteOutput("passed an argument of 1 on key down and 0 on key ");
			context.WriteOutput("up. Omit the command to unbind the key.");
			return false;
		}
		...
		{
			context.WriteOutput("Binding set");
			return true;
		}
		if (retVal == BindingReturnValue::ClearedBinding)
		{
			context.WriteOutput("Binding cleared");
			return true;
		}

		if (retVal == BindingReturnValue::UnknownKey)
			context.WriteOutput("Unrecognized key name: " + Arguments[0]);
		else if (retVal == BindingReturnValue::InvalidArgument)
			context.WriteOutput("Invalid command");
	...
	bool InputCommandProvider::CommandUIButtonPress(const std::vector<std::string>& Arguments, ICommandContext& context)
	{
		if (Arguments.size() < 1)
		{
			context.WriteOutput("Usage: Input.UIButtonPress <btnCode>");
			return false;
		}
