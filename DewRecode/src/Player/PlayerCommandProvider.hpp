...
namespace Player
{
	...
	public:
		...
		bool CommandPrintUid(const std::vector<std::string>& Arguments, std::string& returnInfo);
		bool VariableArmorUpdate(const std::vector<std::string>& Arguments, std::string& returnInfo);
		bool VariableNameUpdate(const std::vector<std::string>& Arguments, std::string& returnInfo);
