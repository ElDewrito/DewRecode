...
namespace Game
{
	...
	void GameCommandProvider::RegisterVariables(ICommandManager* manager)
	{
		...
		VarSkipLauncher = manager->Add(Command::CreateVariableInt("Game", "SkipLauncher", "launcher", "Skip requiring the launcher", eCommandFlagsArchived, 0));
		VarSkipLauncher->ValueIntMin = 0;
		VarSkipLauncher->ValueIntMax = 0;
	}

	bool GameCommandProvider::CommandExit(const std::vector<std::string>& Arguments, std::string& returnInfo)
	{
		this->Exit();
		return true;
	}
	...
	bool GameCommandProvider::CommandInfo(const std::vector<std::string>& Arguments, std::string& returnInfo)
	{
		returnInfo = this->GetInfo();
		return true;
	}
	...
	std::string GameCommandProvider::GetInfo()
	{
		...
		ss << "Server Port: " << std::dec << dorito.ServerCommands->VarPort->ValueInt << std::endl;
	...
	bool GameCommandProvider::CommandVersion(const std::vector<std::string>& Arguments, std::string& returnInfo)
	{
		returnInfo = this->GetVersion();
		return true;
	}
	...
	bool GameCommandProvider::CommandForceLoad(const std::vector<std::string>& Arguments, std::string& returnInfo)
	{
		std::stringstream ss;
	...
	bool GameCommandProvider::CommandMap(const std::vector<std::string>& Arguments, std::string& returnInfo)
	{
		if (Arguments.size() != 1)
		{
		...
		returnInfo = "Loading map \"" + Arguments[0] + "\"...";
	...
	bool GameCommandProvider::CommandGameType(const std::vector<std::string>& Arguments, std::string& returnInfo)
	{
		if (Arguments.size() != 1)
		{
		...
		returnInfo = "Loading game variant \"" + Arguments[0] + "\"...";
	...
	bool GameCommandProvider::CommandStart(const std::vector<std::string>& Arguments, std::string& returnInfo)
	{
