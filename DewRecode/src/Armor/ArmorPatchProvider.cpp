#include "ArmorPatchProvider.hpp"
...
namespace
{
	...
	class ArmorExtension : public Network::PlayerPropertiesExtension<CustomizationData>
	{
	protected:
...
namespace Armor
{
	PatchSet ArmorPatchProvider::GetPatches()
	{
		PatchSet patches("ArmorPatches",
		{
	...
	void ArmorPatchProvider::RegisterCallbacks(IEngine* engine)
	{
		engine->OnEvent("Core", "Engine.TagsLoaded", BIND_CALLBACK(this, &ArmorPatchProvider::SignalRefreshUIPlayerArmor));
		ElDorito::Instance().PlayerPropertiesExtender.Add(std::make_shared<ArmorExtension>());
	}

	void ArmorPatchProvider::SignalRefreshUIPlayerArmor(void* param)
	{
...
namespace
{
	...
	void BuildCustomizationData(CustomizationData* out)
	{
		...
		auto playerVars = ElDorito::Instance().PlayerCommands;
	...
	void UiPlayerModelArmorHook()
	{
		...
		uint32_t uiPlayerBiped = Pointer(0x4FE67A0).Read<uint32_t>();
	...
	void ScoreboardPlayerModelArmorHook()
	{
		uint32_t scoreboardBiped = Pointer(0x504C698).Read<uint32_t>();
