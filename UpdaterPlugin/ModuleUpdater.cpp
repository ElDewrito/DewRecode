...
namespace
{
	...
	bool CommandUpdaterCheck(const std::vector<std::string>& Arguments, std::string& returnInfo)
	{
		std::string branch = Updater->VarUpdaterBranch->ValueString;
		if (Arguments.size() > 0)
			branch = Arguments[0];

		returnInfo = Updater->Check(branch);
		return true;
	}
	...
	void CallbackUpdate(const std::string& boxTag, const std::string& result)
	{
		...
		Engine->ExecuteProcess(L"eldewrito_update_helper", PublicUtils->WidenString("-manifest " + boxTag), 0);
		std::exit(0);
	}
...
namespace Modules
{
	ModuleUpdater::ModuleUpdater() : ModuleBase("Updater")
	{
		Updater = this;
		PublicUtils = utils;
		Engine = engine;

		VarUpdaterBranch = AddVariableString("Branch", "update_branch", "The branch to use for updates.", eCommandFlagsArchived, "main");

		AddCommand("Check", "update_check", "Checks for updates against the server specified in dewrito.json", eCommandFlagsNone, CommandUpdaterCheck, { "branchname The name of the branch to check for updates on" });
	}

	std::string ModuleUpdater::Check(std::string branch)
	{
		std::stringstream ss;
		std::vector<std::string> announceEndpoints;

		GetEndpoints(announceEndpoints, "update");

		std::string currentVer = Engine->GetDoritoVersionString();
		...
		for (auto server : announceEndpoints)
		{
			...
			try
			{
				req = PublicUtils->HttpSendRequest(PublicUtils->WidenString(server + "/channels.json"), L"GET", L"ElDewrito/" + PublicUtils->WidenString(currentVer), L"", L"", L"", NULL, 0);
			...
			try
			{
				req = PublicUtils->HttpSendRequest(PublicUtils->WidenString(manifestJsonPath), L"GET", L"ElDewrito/" + PublicUtils->WidenString(currentVer), L"", L"", L"", NULL, 0);
				if (req.Error != HttpRequestError::None)
				{
			...
			try
			{
				req = PublicUtils->HttpSendRequest(PublicUtils->WidenString(server + "/" + branch + "/manifest.json.sig"), L"GET", L"ElDewrito/" + PublicUtils->WidenString(currentVer), L"", L"", L"", NULL, 0);
				if (req.Error != HttpRequestError::None)
				{
			...
			auto sigValid = PublicUtils->RSAVerifySignature(updateMasterKey, manifestSigB64, (void*)manifest.c_str(), manifest.length());
			if (!sigValid)
			{
			...
			Engine->ShowMessageBox("Update available", "An update for ElDewrito is available, do you want to update now?\nCurrent version: " + currentVer + ", Newer version: " + branchVer, manifestJsonPath, { "Yes", "No" }, CallbackUpdate);
		}
