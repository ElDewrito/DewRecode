...
void onClientMoveSubscriptionEvent(uint64 serverConnectionHandlerID, anyID clientID, uint64 oldChannelID, uint64 newChannelID, int visibility) {
	...
	if (ts3client_getClientVariableAsString(serverConnectionHandlerID, clientID, CLIENT_NICKNAME, &name) != ERROR_ok)
		return;
	if (engine != nullptr)
		engine->PrintToConsole("Client " + std::string(name) + " has joined the VoIP chat!");
	ts3client_freeMemory(name);  /* Release dynamically allocated memory only if function succeeded */
}
...
void onClientMoveTimeoutEvent(uint64 serverConnectionHandlerID, anyID clientID, uint64 oldChannelID, uint64 newChannelID, int visibility, const char* timeoutMessage) {
	if (engine != nullptr)
		engine->PrintToConsole("ClientID " + std::to_string(clientID) + " timedout with message " + std::string(timeoutMessage));
}
...
#ifdef _DEBUG
		if (engine != nullptr)
			engine->PrintToConsole("Client " + std::string(name) + " starts talking.");
#endif
		// TODO1: MemberList::Instance().ShowPlayerTalkEvent(std::string(name));
	}
	else {
#ifdef _DEBUG
		if (engine != nullptr)
			engine->PrintToConsole("Client " + std::string(name) + " stopped talking.");
#endif
		// TODO1: MemberList::Instance().HidePlayerTalkEvent(std::string(name));
	}
	ts3client_freeMemory(name);  /* Release dynamically allocated memory only if function succeeded */
}
...
void onUserLoggingMessageEvent(const char* logMessage, int logLevel, const char* logChannel, uint64 logID, const char* logTime, const char* completeLogString) {
	...
	if (engine != nullptr)
		engine->PrintToConsole(completeLogString);
...
DWORD WINAPI StartTeamspeakClient(Modules::ModuleVoIP& voipModule)
{
	...
	if (engine != nullptr)
		engine->PrintToConsole("Starting VoIP client...");
	...
	if ((error = ts3client_initClientLib(&funcs, NULL, LogType_FILE | LogType_CONSOLE | LogType_USERLOGGING, NULL, "")) != ERROR_ok)
	{
		...
		{
			if (engine != nullptr)
				engine->PrintToConsole("Error initialzing clientlib: " + std::string(errormsg));
			ts3client_freeMemory(errormsg);
		}
		return 1;
	}
	...
	{
		if (engine != nullptr)
			engine->PrintToConsole("Error spawning server connection handler: " + std::to_string(error));
		return 1;
	}
	...
	{
		if (engine != nullptr)
			engine->PrintToConsole("Error getting default capture mode: " + std::to_string(error));
		return 1;
	}
#ifdef _DEBUG
	if (engine != nullptr)
		engine->PrintToConsole("Default capture mode: " + std::string(mode));
#endif
	/* Get default capture device */
	if ((error = ts3client_getDefaultCaptureDevice(mode, &device)) != ERROR_ok)
	{
		if (engine != nullptr)
			engine->PrintToConsole("Error getting default capture device: " + std::to_string(error));
		return 1;
	}
#ifdef _DEBUG
	if (engine != nullptr)
		engine->PrintToConsole("Default capture device: " + std::string(device[0]) + " " + std::string(device[1]));
#endif
	...
	{
		if (engine != nullptr)
			engine->PrintToConsole("Error opening capture device: " + std::to_string(error));
	}
	...
	{
		if (engine != nullptr)
			engine->PrintToConsole("Error getting default playback mode: " + std::to_string(error));
		return 1;
	}
#ifdef _DEBUG
	if (engine != nullptr)
		engine->PrintToConsole("Default playback mode:  " + std::string(mode));
#endif
	/* Get default playback device */
	if ((error = ts3client_getDefaultPlaybackDevice(mode, &device)) != ERROR_ok)
	{
		if (engine != nullptr)
			engine->PrintToConsole("Error getting default playback device: " + std::to_string(error));
		return 1;
	}
#ifdef _DEBUG
	if (engine != nullptr)
		engine->PrintToConsole("Default playback device: " + std::string(device[0]) + " " + std::string(device[1]));
#endif
	...
	{
		if (engine != nullptr)
			engine->PrintToConsole("Error opening playback device: " + std::to_string(error));
	}
	...
	if (readIdentity(identity) != 0) {
		...
		{
			if (engine != nullptr)
				engine->PrintToConsole("Error creating identity: " + std::to_string(error));
			return 0;
		}
		if (strlen(id) >= IDENTITY_BUFSIZE)
		{
			if (engine != nullptr)
				engine->PrintToConsole("Not enough bufsize for identity string");
			return 0;
		}
		strcpy(identity, id);
...
#ifdef _DEBUG
	if (engine != nullptr)
		engine->PrintToConsole("Using identity: " + std::string(identity));
#endif
	...
	if (engine != nullptr)
		engine->PrintToConsole("Eldewrito VoIP initialized and running!");

	/* Query and print client lib version */
	if ((error = ts3client_getClientLibVersion(&version)) != ERROR_ok)
	{
		if (engine != nullptr)
			engine->PrintToConsole("Failed to get VoIP clientlib version: " + std::to_string(error));
		return 1;
	}
#ifdef _DEBUG
	if (engine != nullptr)
		engine->PrintToConsole("VoIP Client lib version: " + std::string(version));
#endif
	...
	while (!abort) {
		...
		if (voipModule.VarVoIPPushToTalk->ValueInt == 1)
		{
			if ((error = ts3client_setClientSelfVariableAsInt(scHandlerID, CLIENT_INPUT_DEACTIVATED, voipModule.VarVoIPTalk->ValueInt ? INPUT_ACTIVE : INPUT_DEACTIVATED)) != ERROR_ok)
			{
				...
				{
					if (engine != nullptr)
						engine->PrintToConsole("Error toggling push-to-talk: " + std::string(errorMsg));
					ts3client_freeMemory(errorMsg);
				}
			}
			...
			if (ts3client_flushClientSelfUpdates(scHandlerID, NULL) != ERROR_ok)
			{
				...
				{
					if (engine != nullptr)
						engine->PrintToConsole("Error flushing after toggling push-to-talk: " + std::string(errorMsg));
					ts3client_freeMemory(errorMsg);
				}
			}
		}
	...
	*/
	...
	{
		if (engine != nullptr)
			engine->PrintToConsole("Error stopping VoIP connection: " + std::to_string(error));
		return 1;
	}
	...
	{
		if (engine != nullptr)
			engine->PrintToConsole("Error destroying VoIP clientlib: " + std::to_string(error));
		return 1;
	}
	...
	{
		if (engine != nullptr)
			engine->PrintToConsole("Failed to destroy VoIP clientlib: " + std::to_string(error));
		return 1;
	}
	...
	if (engine != nullptr)
		engine->PrintToConsole("Stopped VoIP client");
	return 0;
}
